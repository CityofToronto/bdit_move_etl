...
def get_pair_sql(a_1, a_2, centreline_id, geom):
  '''
...
INSERT INTO counts_new.arteries_groups (
  ...
  ({a_1}, 4, {a_1}, 1, {centreline_id}, '{geom}'),
  ({a_2}, 4, {a_1}, 1, {centreline_id}, '{geom}');'''.format(
    a_1=a_1,
    a_2=a_2,
  ...
  def main():
    ...
    for row in reader:
      ...
      if a_1 not in paired and a_2 not in paired:
        sql = get_pair_sql(a_1, a_2, centreline_id, geom)
        print(sql)
        paired.add(a_1)
        paired.add(a_2)
