...
CREATE TABLE IF NOT EXISTS counts_new.nodes_centreline (
...
CREATE UNIQUE INDEX IF NOT EXISTS nodes_centreline_link_id ON counts_new.nodes_centreline (link_id);

TRUNCATE TABLE counts_new.nodes_centreline;

-- Case 1: corrected node geometry has close spatial match (10m) to intersection geometry
INSERT INTO counts_new.nodes_centreline (
  SELECT
    ncor.link_id,
    1 AS match_on_case,
    ci."centrelineType",
    ci."centrelineId",
    ncor.geom
  FROM counts.nodes_corrected ncor
  JOIN LATERAL (
    SELECT
      "centrelineType",
      "centrelineId",
...
INSERT INTO counts_new.nodes_centreline (
  SELECT
    ...
    ncor.geom
  FROM counts.nodes_corrected ncor
  LEFT JOIN counts_new.nodes_centreline ncen USING (link_id)
  JOIN LATERAL (
    SELECT
      "centrelineType",
      "centrelineId",
...
CREATE MATERIALIZED VIEW IF NOT EXISTS counts.nodes_centreline AS
  SELECT * FROM counts_new.nodes_centreline;
CREATE UNIQUE INDEX IF NOT EXISTS nodes_centreline_link_id ON counts.nodes_centreline (link_id);
CREATE INDEX IF NOT EXISTS nodes_centreline_srid2952_geom ON counts.nodes_centreline USING gist (ST_Transform(geom, 2952));

REFRESH MATERIALIZED VIEW CONCURRENTLY counts.nodes_centreline;
