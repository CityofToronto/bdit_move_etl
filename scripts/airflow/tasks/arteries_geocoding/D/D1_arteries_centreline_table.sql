...
CREATE TABLE IF NOT EXISTS counts2_new.arteries_centreline (
...
CREATE UNIQUE INDEX IF NOT EXISTS arteries_centreline_arterycode ON counts2_new.arteries_centreline (arterycode);

TRUNCATE TABLE counts2_new.arteries_centreline;

-- Case 1: traffic signals
INSERT INTO counts2_new.arteries_centreline (
  SELECT
    apc.arterycode,
    1 AS match_on_case,
    apc."centrelineType",
    apc."centrelineId",
    apc.geom
  FROM counts2.arteries_px_centreline apc
  LEFT JOIN counts2_new.arteries_centreline ac USING (arterycode)
...
INSERT INTO counts2_new.arteries_centreline (
  ...
  FROM counts2.arteries_manual_corr_type1 amct1
...
INSERT INTO counts2_new.arteries_centreline (
  SELECT
    ...
    amct2."centrelineId",
    ci.geom as geom
  FROM counts2.arteries_manual_corr_type2 amct2
...
INSERT INTO counts2_new.arteries_centreline (
  ...
  FROM counts2.arteries_single_node asn
  JOIN centreline.midblocks cm USING ("centrelineId")
  LEFT JOIN counts2_new.arteries_centreline ac USING (arterycode)
...
INSERT INTO counts2_new.arteries_centreline (
  ...
  FROM counts2.arteries_double_node_midblocks_single adnms
  JOIN centreline.midblocks cm USING ("centrelineId")
  LEFT JOIN counts2_new.arteries_centreline ac USING (arterycode)
  WHERE ac.arterycode IS NULL
);
...
INSERT INTO counts2_new.arteries_centreline (
  ...
  FROM counts2.arteries_double_node_midblocks_multi_best adnmmb
  JOIN centreline.midblocks cm USING ("centrelineId")
  LEFT JOIN counts2_new.arteries_centreline ac USING (arterycode)
  WHERE ac.arterycode IS NULL
);
...
INSERT INTO counts2_new.arteries_centreline (
  ...
  LEFT JOIN counts2_new.arteries_centreline am USING (arterycode)
...
INSERT INTO counts2_new.arteries_centreline (
  ...
  LEFT JOIN counts2_new.arteries_centreline ac ON ad."ARTERYCODE" = ac.arterycode
  WHERE ac.arterycode IS NULL
);
